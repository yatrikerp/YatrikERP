...
async function createDepotAndUser(depotInfo, depotType) {
  try {
...
async function setupComprehensiveDepots() {
  ...
  for (const depot of depotData.mainDepots) {
    const result = await createDepotAndUser(depot, 'main');
    if (result.success) {
  ...
  for (const depot of depotData.subDepots) {
    const result = await createDepotAndUser(depot, 'sub');
    if (result.success) {
      results.sub.success++;
      credentials.push({
        type: 'Sub Depot',
  ...
  for (const depot of depotData.operatingCenters) {
    const result = await createDepotAndUser(depot, 'operating');
    if (result.success) {
