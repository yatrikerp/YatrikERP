...
async function scheduleAllTripsAndRoutes() {
  try {
    ...
    const routes = await Route.find({ status: 'active' }).populate('depotId');
    ...
    for (let day = 0; day < daysToSchedule; day++) {
      ...
      routes.forEach((route, routeIndex) => {
        ...
        const depotBuses = buses.filter(bus => 
          bus.depotId && route.depotId && 
          bus.depotId._id.toString() === route.depotId._id.toString()
        );

        if (depotBuses.length === 0) {
          console.log(`   ⚠️ No buses found for route ${route.routeName} (${route.depotId?.depotName})`);
          return;
        }
