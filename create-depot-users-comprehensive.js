...
async function createDepotUser(depot, depotType) {
  try {
    const email = generateDepotEmail(depot.abbr, depotType);
    const password = generateDepotPassword(depot.abbr);
    const username = `${depot.abbr.toLowerCase()}-${depotType}`;

    console.log(`🔧 Creating ${depotType} depot user for ${depot.name}...`);
    console.log(`   Email: ${email}`);
    console.log(`   Password: ${password}`);
    console.log(`   Username: ${username}`);

    // First, create the depot in the database
    const depotData = {
      depotCode: depot.abbr,
      depotName: depot.name,
      location: {
        address: `${depot.name} Bus Station`,
        city: depot.name,
        state: 'Kerala',
        pincode: '680000'
      },
      contact: {
        phone: depot.phoneNo || depot.officeNo || '0000000000',
        email: email,
        manager: { name: `${depot.name} Manager` }
      },
      capacity: {
    ...
    const response = await axios.post('http://localhost:5000/api/admin/depots', depotData, {
      headers: {
    ...
    if (response.data.success) {
      console.log(`✅ Successfully created ${depotType} depot: ${depot.name}`);
      console.log(`   Depot ID: ${response.data.depot?._id || 'N/A'}`);
      console.log(`   User ID: ${response.data.user?._id || 'N/A'}`);
      return { success: true, depot: depot.name, email, password };
    } else {
      console.log(`❌ Failed to create ${depotType} depot: ${depot.name}`);
      console.log(`   Error: ${response.data.error || 'Unknown error'}`);
      return { success: false, depot: depot.name, error: response.data.error };
    }

  } catch (error) {
    if (error.response?.status === 400 && error.response?.data?.error?.includes('already exists')) {
      console.log(`⚠️  ${depotType} depot ${depot.name} already exists - skipping`);
      return { success: true, depot: depot.name, skipped: true };
    } else {
      console.log(`❌ Error creating ${depotType} depot ${depot.name}:`, error.response?.data || error.message);
      return { success: false, depot: depot.name, error: error.response?.data || error.message };
    }
  }
}

async function createAllDepotUsers() {
  console.log('🚀 Starting comprehensive depot user creation...\n');
  
  const results = {
    main: { success: 0, failed: 0, skipped: 0 },
    sub: { success: 0, failed: 0, skipped: 0 },
    operating: { success: 0, failed: 0, skipped: 0 }
  };
  ...
  for (const depot of depotData.mainDepots) {
    const result = await createDepotUser(depot, 'main');
    if (result.success) {
      if (result.skipped) {
  ...
  for (const depot of depotData.subDepots) {
    const result = await createDepotUser(depot, 'sub');
    if (result.success) {
      if (result.skipped) {
        results.sub.skipped++;
      } else {
  ...
  for (const depot of depotData.operatingCenters) {
    const result = await createDepotUser(depot, 'operating');
    if (result.success) {
      if (result.skipped) {
        results.operating.skipped++;
      } else {
  ...
  // Summary
  console.log('\n📊 DEPOT CREATION SUMMARY:');
  console.log('=====================================');
  console.log(`Main Depots:     ${results.main.success} created, ${results.main.skipped} skipped, ${results.main.failed} failed`);
  console.log(`Sub Depots:      ${results.sub.success} created, ${results.sub.skipped} skipped, ${results.sub.failed} failed`);
  console.log(`Operating Centers: ${results.operating.success} created, ${results.operating.skipped} skipped, ${results.operating.failed} failed`);
  console.log(`Total:           ${results.main.success + results.sub.success + results.operating.success} created, ${results.main.skipped + results.sub.skipped + results.operating.skipped} skipped, ${results.main.failed + results.sub.failed + results.operating.failed} failed`);
  ...
  console.log('✅ Depot user creation completed!');
...
// Run the script
createAllDepotUsers().catch(console.error);
