...
class _DashboardTabState extends State<DashboardTab> {
  ...
  Future<void> _loadDashboardData() async {
    ...
    try {
      ...
      try {
        final walletResponse = await apiService.get('/api/passenger/wallet/balance');
        if (walletResponse['success'] == true) {
          setState(() {
            walletBalance = (walletResponse['balance'] ?? 0.0).toDouble();
          });
        }
      } catch (walletError) {
        print('⚠️ Error loading wallet balance: $walletError');
  ...
  Widget _buildRouteCard(Map<String, dynamic> route) {
    ...
    return InkWell(
      onTap: () {
        // Navigate to search tab with pre-filled route data
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 4),
            if (route['fare'] != null || route['minFare'] != null)
              Text(
                route['fare'] ?? 'From ₹${route['minFare']}',
                style: const TextStyle(
                  fontSize: 11,
                  color: Colors.green,
                  fontWeight: FontWeight.w600,
            ...
            Text(
              route['frequency'] ?? 'Route: ${route['routeName'] ?? route['routeNumber'] ?? 'N/A'}',
              style: const TextStyle(
                fontSize: 10,
                color: AppColors.text600,
              ),
