...
import {
  ...
  Star, Award, TrendingUp, Activity, Snowflake
} from 'lucide-react';
...
const DriverDashboard = () => {
  ...
  const fetchDriverData = async () => {
    try {
      const response = await apiFetch('/api/driver/profile');
      if (response.success) {
  ...
  const fetchTrips = async () => {
    try {
      ...
      // Fetch today's trips
      const todayResponse = await fetch(`/api/trips/driver/${driverId}?date=${today}&status=scheduled,running`, {
        headers: {
      ...
      const upcomingResponse = await fetch(`/api/trips/driver/${driverId}?status=scheduled`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
      ...
      const completedResponse = await fetch(`/api/trips/driver/${driverId}?status=completed`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-orange-600">{stats.totalDistance.toFixed(1)} km</p>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="text-center">
              <div className="flex items-center justify-center mb-2">
                ...
                <span className="text-2xl font-bold text-gray-900">{stats.completedTrips}</span>
              </div>
            ...
            <div className="text-center">
              <div className="flex items-center justify-center mb-2">
                ...
                <span className="text-2xl font-bold text-gray-900">{stats.totalDistance.toFixed(0)}km</span>
              </div>
