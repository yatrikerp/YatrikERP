...
async function createDepotAndUser(depotInfo, depotType, adminUserId) {
  try {
...
async function setupComprehensiveDepots() {
  ...
  for (const depot of depotData.mainDepots) {
    const result = await createDepotAndUser(depot, 'main', adminUser._id);
    if (result.success) {
  ...
  for (const depot of depotData.subDepots) {
    const result = await createDepotAndUser(depot, 'sub', adminUser._id);
    if (result.success) {
      results.sub.success++;
      credentials.push({
        type: 'Sub Depot',
  ...
  for (const depot of depotData.operatingCenters) {
    const result = await createDepotAndUser(depot, 'operating', adminUser._id);
    if (result.success) {
