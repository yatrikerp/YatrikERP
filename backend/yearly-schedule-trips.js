...
async function autoScheduleTripsYearly() {
  try {
    ...
    // Group buses by depot name (FIXED: Use depot name instead of ID)
    ...
    console.log(`📊 Crew grouped by depot:`);
    console.log(`   Drivers: ${Object.keys(driversByDepotName).length} depot groups`);
    console.log(`   Conductors: ${Object.keys(conductorsByDepotName).length} depot groups`);
    ...
    for (let dayOffset = 0; dayOffset < DAYS_TO_SCHEDULE; dayOffset++) {
      ...
      for (const route of routes) {
        // Get route's depot - FIXED: Handle different depot structures
        ...
        const actualTrips = Math.min(tripFrequency, availableBuses.length, timeSlots.length);
    ...
    // Insert new trips in optimized batches
