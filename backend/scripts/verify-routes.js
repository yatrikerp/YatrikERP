...
const verifyRoutes = async () => {
  try {
    ...
    const sampleRoutes = await Route.find({ isActive: true })
      .populate('depot.depotId', 'depotName')
      .limit(10)
    ...
    // Group by bus type
    const busTypeStats = await Route.aggregate([
      { $match: { isActive: true } },
      { $group: { _id: '$busType', count: { $sum: 1 } } },
      { $sort: { count: -1 } }
    ]);
    
    console.log('🚌 Routes by Bus Type:');
    busTypeStats.forEach(stat => {
      console.log(`   ${stat._id}: ${stat.count} routes`);
    });
    
    // Group by depot
    const depotStats = await Route.aggregate([
      { $match: { isActive: true } },
