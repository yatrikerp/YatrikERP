  ...
  try {
    ...
    try {
      ...
      for (const driver of drivers.slice(0, 10)) {
        try {
          const fatigueScore = await autonomousScheduler.calculateFatigueScore(
            driver,
            "driver",
          );
          if (fatigueScore > 70) {
    ...
    const lowOccupancyRoutes = await Trip.aggregate([
      ...
      { $unwind: "$route" },
      {
        $group: {
          ...
          avgOccupancy: {
            $avg: { $divide: [{ $size: "$bookings" }, "$totalSeats"] },
          },
          tripCount: { $sum: 1 },
        },
      },
  ...
  try {
    const models = [
      ...
      },
      {
        ...
        type: "Ensemble",
        description:
          "Classifies route performance and optimization opportunities",
        status: "active",
        accuracy: 91.2,
...
router.post("/ml/predict", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to run prediction" });
  }
});
...
router.post("/ml/train", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to initiate training" });
  }
});
  ...
  try {
    ...
    const [
      ...
      Bus.find({
        ...
        .select(
          "busNumber capacity status depotId fuelCapacity odometerReading",
        )
        .lean(),
      ...
      Conductor.find({ status: "active" })
        ...
        .lean(),
      Depot.find({ status: "active" }).select("name location capacity").lean(),
      Booking.find({
    ...
    console.log(
      `✓ Aggregated: ${routes.length} routes, ${buses.length} buses, ${drivers.length} drivers, ${conductors.length} conductors, ${depots.length} depots`,
    );
    ...
    if (buses.length === 0 || drivers.length === 0 || conductors.length === 0) {
      return res.status(400).json({
        success: false,
        message: `Insufficient resources: ${buses.length} buses, ${drivers.length} drivers, ${conductors.length} conductors. Need at least 1 of each.`,
        data: {
          schedulesGenerated: 0,
          ...
          conflicts: [
            {
              ...
              message: `Missing resources: ${buses.length === 0 ? "buses " : ""}${drivers.length === 0 ? "drivers " : ""}${conductors.length === 0 ? "conductors" : ""}`,
              severity: "high",
            },
          ],
          summary: {
    ...
    routes.forEach((route) => {
      const historicalDemand = bookings.filter(
        (b) => b.tripId?.routeId?.toString() === route._id.toString(),
      ).length;
      ...
      const predicted = Math.floor(
        historicalDemand * baseGrowth * seasonalFactor,
      );
    ...
    for (const [routeId, tripReq] of Object.entries(tripRequirements)) {
      ...
      // Route has depot.depotId (ObjectId), not depotId directly
      ...
      for (const timeSlot of tripReq.timeSlots) {
        ...
        for (const driver of drivers) {
          ...
          // Check depot match
          if (routeDepotId && driverDepotId && routeDepotId !== driverDepotId)
            continue;

          // Calculate fatigue score
          const hoursWorked = driverSchedule.reduce(
            (sum, slot) => sum + (slot.end - slot.start),
            0,
          );
          const consecutiveDays = driverSchedule.length;
          const nightShifts = driverSchedule.filter(
            (s) => s.start >= 22 || s.start < 6,
          ).length;

          driverFatigue =
            hoursWorked * 0.6 + consecutiveDays * 0.3 + nightShifts * 0.1;

          // Check eligibility (max 48 hrs/week, fatigue < 70)
          if (hoursWorked >= 48 || driverFatigue > 70) continue;

          // Check time conflict
          const hasConflict = driverSchedule.some(
            (slot) =>
              (timeSlot.hour >= slot.start && timeSlot.hour < slot.end) ||
              (arrivalHour > slot.start && arrivalHour <= slot.end),
          );
        ...
        for (const conductor of conductors) {
          // With .lean(), depotId is either ObjectId or populated object
          const conductorDepotId =
            conductor.depotId?._id?.toString() || conductor.depotId?.toString();
          const conductorSchedule =
            usedConductors.get(conductor._id.toString()) || [];

          // Check depot match
          if (
            routeDepotId &&
            conductorDepotId &&
            routeDepotId !== conductorDepotId
          )
            continue;

          // Calculate fatigue
          const hoursWorked = conductorSchedule.reduce(
            (sum, slot) => sum + (slot.end - slot.start),
            0,
          );
          ...
          // Check time conflict
          const hasConflict = conductorSchedule.some(
            (slot) =>
              (timeSlot.hour >= slot.start && timeSlot.hour < slot.end) ||
              (arrivalHour > slot.start && arrivalHour <= slot.end),
          );
    ...
    const routeCoverage =
      (new Set(schedule.map((s) => s.route)).size / routes.length) * 100;
    const avgFatigue =
      schedule.reduce((sum, s) => sum + s.driverFatigue, 0) / schedule.length;
    ...
    const conflictPenalty =
      conflictSummary.high * 10 +
      conflictSummary.medium * 5 +
      conflictSummary.low * 2;
    ...
    console.log(
      `📊 Generated ${result.schedulesGenerated} schedules with ${Math.round(optimizationScore)}% optimization`,
    );
...
router.get("/predictive/demand", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to forecast demand" });
  }
});
  ...
  try {
    ...
    const forecast = {
      ...
      breakdown: {
        ...
        premiumServices: Math.floor(historicalRevenue * 0.1),
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to forecast revenue" });
  }
});
  ...
  try {
    ...
    buses.forEach((bus) => {
      ...
      if (daysUntilMaintenance <= 30 || kmUntilMaintenance <= 3000) {
        predictions.push({
          ...
          kmUntilMaintenance,
          priority:
            daysUntilMaintenance <= 7 || kmUntilMaintenance <= 1000
              ? "high"
              : "medium",
          estimatedCost: Math.floor(Math.random() * 5000) + 2000,
          recommendedDate: new Date(
            now.getTime() + daysUntilMaintenance * 24 * 60 * 60 * 1000,
          ),
        });
      }
    });
    ...
    res.json({
      ...
      data: {
        ...
        estimatedTotalCost: predictions.reduce(
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to forecast maintenance" });
  }
});
