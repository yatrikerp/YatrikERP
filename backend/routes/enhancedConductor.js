...
router.get(
  "/trips",
  auth,
  requireRole(["conductor", "admin", "depot_manager"]),
  async (req, res) => {
    try {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (
        trip.conductorId &&
        trip.conductorId._id.toString() !== req.user._id.toString()
      ) {
      ...
      // Get conductor info
      const conductor = await User.findById(req.user._id).select(
        "name email phone",
      );
      ...
      res.json({
        ...
        data: {
          ...
          summary: {
            ...
            validatedPassengers: passengers.filter(
              (p) => p.state === "validated",
            ).length,
            pendingPassengers: passengers.filter((p) => p.state === "active")
              .length,
          },
        },
      });
